...
import '../settings/embedded_proxy_controller.dart';
import '../settings/ghost_mode_controller.dart';
...
class _ChatListViewState extends State<ChatListView>
  ...
  void initState() {
    ...
    unawaited(EmbeddedProxyController.instance.refresh());
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            GestureDetector(
              behavior: HitTestBehavior.opaque,
              onTap: () => unawaited(_showGhostModeSheet()),
              child: SizedBox(
                width: AppMetric.hitTarget,
                height: AppMetric.hitTarget,
                child: AnimatedBuilder(
                  animation: GhostModeController.instance,
                  builder: (context, _) => AppIcon(
                    HeroAppIcons.eyeSlash,
                    size: AppIconSize.toolbar,
                    color:
                        GhostModeController.instance.enabled ||
                            GhostModeController.instance.networkPaused
                        ? AppTheme.brand
                        : c.textPrimary,
                  ),
                ),
        ...
        AnimatedSwitcher(
          ...
          child: _searchExpanding
              ? _inlineSearchBar(key: const ValueKey('search'))
              : Padding(
  ...
  Future<void> _showGhostModeSheet() async {
    await showModalBottomSheet<void>(
      ...
      builder: (_) => const _GhostModeSheet(),
    );
  }

  Future<void> _openSearch() async {
    setState(() => _searchExpanding = true);
    await Navigator.of(
      context,
    ).push(MaterialPageRoute(builder: (_) => const SearchView()));
    if (mounted) setState(() => _searchExpanding = false);
  }

  Widget _inlineSearchBar({required Key key}) {
    final c = context.colors;
    return Container(
      key: key,
      height: 36,
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'Buscar',
              style: GoogleFonts.inter(fontSize: 15, color: c.textTertiary),
            ),
          ),
        ],
      ),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            AnimatedBuilder(
              animation: EmbeddedProxyController.instance,
              builder: (context, _) {
                final proxy = EmbeddedProxyController.instance;
                return GestureDetector(
                  behavior: HitTestBehavior.opaque,
                  onTap: proxy.busy
                      ? null
                      : () => unawaited(proxy.toggle()),
                  child: SizedBox(
                    height: AppMetric.menuRowHeight,
                    child: Padding(
                      padding: const EdgeInsets.symmetric(
                      ...
                      child: Row(
                        children: [
                          ...
                          if (proxy.busy)
                            ...
                            Switch(
                              value: proxy.enabled,
                              onChanged: (_) => unawaited(proxy.toggle()),
...
/// Modo Ghost bottom sheet: two independent switches — suppress read
/// receipts, and fully pause the app's network connection.
class _GhostModeSheet extends StatefulWidget {
  const _GhostModeSheet();

  @override
  State<_GhostModeSheet> createState() => _GhostModeSheetState();
}

class _GhostModeSheetState extends State<_GhostModeSheet> {
  @override
  void initState() {
    super.initState();
    GhostModeController.instance.addListener(_onChange);
  }

  @override
  void dispose() {
    GhostModeController.instance.removeListener(_onChange);
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    final c = context.colors;
    final ghost = GhostModeController.instance;
    return SafeArea(
      top: false,
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _ghostRow(
              ...
              title: 'Modo Ghost',
              subtitle:
                  'Lê as mensagens sem enviar confirmação de leitura (✓✓)',
              value: ghost.enabled,
              onChanged: (value) => ghost.enabled = value,
            ),
            const Padding(
            ...
            _ghostRow(
              ...
              title: 'Cortar internet do app',
              subtitle:
                  'O app fica sem conexão até você desativar — mensagens só chegam depois disso, sem mexer no wi-fi/dados do celular',
              value: ghost.networkPaused,
              onChanged: (value) => unawaited(ghost.setNetworkPaused(value)),
            ),
          ],
        ),
      ),
        ...
        children: [
          Padding(
            ...
            child: AppIcon(
              icon,
              size: 22,
              color: value ? AppTheme.brand : c.textSecondary,
            ),
          ),
