...
import 'self_destruct_protocol.dart';
...
class MessageBubble extends StatefulWidget {
  ...
  final ValueChanged<ChatMessage>? onSelfDestructExpired;
...
class _MessageBubbleState extends State<MessageBubble>
  ...
  Widget _textBubble(String text, bool outgoing) {
    final selfDestructPayload = extractSelfDestructPayload(text);
    if (selfDestructPayload != null) {
      return _SelfDestructBubble(
        key: ValueKey('selfDestruct-${message.id}'),
        content: selfDestructPayload,
        outgoing: outgoing,
        bubbleColor: outgoing ? _outgoingBubbleColor : _incomingBubbleColor,
        textColor: outgoing ? _outgoingTextColor : _incomingTextColor,
        maxWidth: _bubbleMaxWidth(),
        onExpired: () => widget.onSelfDestructExpired?.call(message),
      );
    }
    final c = context.colors;
...
/// Self-destruct text bubble: locked by default, reveals [content] for 5
/// seconds after a tap (with a countdown), then calls [onExpired] so the
/// caller can delete the real message.
class _SelfDestructBubble extends StatefulWidget {
  const _SelfDestructBubble({
    super.key,
  ...
  @override
  State<_SelfDestructBubble> createState() => _SelfDestructBubbleState();
}

enum _SelfDestructPhase { locked, revealed, expired }

class _SelfDestructBubbleState extends State<_SelfDestructBubble> {
  static const _revealSeconds = 5;
  _SelfDestructPhase _phase = _SelfDestructPhase.locked;
  ...
  void _reveal() {
    if (_phase != _SelfDestructPhase.locked) return;
    setState(() {
      _phase = _SelfDestructPhase.revealed;
      _secondsLeft = _revealSeconds;
    });
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      if (_secondsLeft <= 1) {
        timer.cancel();
        setState(() {
          _phase = _SelfDestructPhase.expired;
          _secondsLeft = 0;
        });
        widget.onExpired();
        return;
  ...
  @override
  Widget build(BuildContext context) {
    final foreground = widget.textColor;
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onTap: _reveal,
      child: Container(
        ...
        child: switch (_phase) {
          _SelfDestructPhase.locked => Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              AppIcon(HeroAppIcons.lock, size: 18, color: foreground),
              const SizedBox(width: 8),
              Flexible(
                child: Text(
                  'Mensagem autodestrutiva — toque para ver',
                  style: TextStyle(fontSize: 15, color: foreground),
                ),
              ),
            ],
          ),
          _SelfDestructPhase.revealed => Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            mainAxisSize: MainAxisSize.min,
            children: [
              Text(
              ...
              Row(
                ...
                children: [
                  SizedBox(
                    ...
                    child: CircularProgressIndicator(
                      strokeWidth: 2,
                      value: _secondsLeft / _revealSeconds,
          ...
          _SelfDestructPhase.expired => Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              AppIcon(
                HeroAppIcons.trash,
